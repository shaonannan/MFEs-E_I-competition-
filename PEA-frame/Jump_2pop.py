...
#     mEY[i] = 2.5#3.11*0.74#
#     mIY[i] = 2.0#2.30*0.74#
#     fE[i],fI[i] = 0.013/0.74,0.013/0.74#0.01316/0.64,0.013145/0.64#
# 20/04/2020
for i in range(NPATCH):
    mEY[i] = 3.11*0.74
    mIY[i] = 2.30*0.74#50*0.82#1.504
    fE[i],fI[i] = 0.01316/0.74,0.013145/0.74
mEY1, mIY1, fE1, fI1 = np.zeros(NPATCH), np.zeros(NPATCH), np.zeros(NPATCH), np.zeros(NPATCH)
# 20/04/2020
for i in range(NPATCH):
    mEY1[i] = 3.11*0.82
    mIY1[i] = 2.30*0.82#50*0.82#1.504
    fE1[i],fI1[i] = 0.01316/0.74,0.013145/0.74
...
factor,ratio = 0.102,4.8#0.102
LEE[0,1] = 0.285*factor*10.0#12.0
LEE[1,0] = 0.285*factor*10.0#12.0
LIE[0,1] = 0.285*factor*10.0*ratio
LIE[1,0] = 0.285*factor*10.0*ratio
#
#LEI[0, 1] = 0.01*100*0.0 # 
#LEI[1, 0] = 0.01*100*0.0 # 
#LII[0, 1] = 0.01*100*0.0 # 
#LII[1, 0] = 0.01*100*0.0 # 
# >>>>>>>>>>>>>>>>>>> Initiation Finished >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
def cortical_to_cortical_connection(background_population_dict, internal_population_dict, DEE, DIE, DEI, DII, LEE, LIE, LEI, LII,
                                    fE, fI, delay):
    connection_list = []
...
connection_list = cortical_to_cortical_connection(background_population_dict, internal_population_dict, DEE, DIE, DEI,
                                                  DII, LEE, LIE, LEI, LII, fE, fI, 0.0)

"""
"""
tau_d = 128.0
simulation = Simulation(population_list, connection_list, Net_settings, Cell_type_num, DEE+LEE/tau_d, DIE+LIE/tau_d, DEI+LEI/tau_d, DII+LII/tau_d, LEE, LIE,
...
scio.savemat(fileparamname,
             {'DEE': DEE, 'DEI': DEI, 'DIE': DIE, 'DII': DII, 'LEE': LEE, 'LIE': LIE, 'mEY': mEY, 'mIY': mIY,
              'fE': fE, 'fI': fI})
...
scio.savemat(filelrname, {'NMDAEbin_ra': NMDAEbin_ra, 'NMDAIbin_ra': NMDAIbin_ra})
plt.figure(1)
plt.subplot(211)
plt.plot(mEbin_ra[:,0])
plt.subplot(212)
