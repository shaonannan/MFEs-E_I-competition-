...
class Connection(object):
    ...
    Output pair
    """
    def __init__(self,pre,post,nsyn,nsyn_post,weights,probs,conn_type, v_min,v_max,dv,**kwargs):
    ...
    def initialize_delay_queue(self):
        ...
        if self.pre_population.type =='Recurrent':
            self.delay_queue = np.core.numeric.ones(self.delay_ind+1)*self.pre_population.curr_firing_rate
        elif self.pre_population.type == 'External':
            self.delay_queue = np.core.numeric.ones(self.delay_ind+1)*self.pre_population.curr_firing_rate
        else:
            raise Exception('Unrecognized source type! (for time delay)')
    ...
    def update(self):
        ...
        self.delay_queue[0] = self.pre_population.curr_firing_rate
